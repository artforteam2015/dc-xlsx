...
var XLSX = Promise.promisifyAll(require('xlsx'));
...
module.exports = {
    ...
    parseFileAsync: function(path, options, callback) {
        XLSX.readFileAsync(path, options)
        .then(function(xlsxFile) {
            pmap(xlsxFile.Sheets, function(sheet, name) {
                XLSX.utils.sheet_to_jsonAsync(sheet, { header: 1, raw: true })
